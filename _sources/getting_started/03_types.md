...
![Error Message](../images/03_01.png)
...
This is just a quick example of a few of the types we'll see, some other ones we will see are specific to Unity. Such as Rigidbody2D, BoxCollider2D, Vector2, Transform, and GameObject.
...
# Challenge
This should only take a minute if you want to do it, but just think about how you would create a variable that cannot be accessed by any other class, and only accepts numerical values. Also, it has a default value of 27.1. Next we want a variable that can be accessed by derived classes and only accepts text. And that's about it for types (for now).